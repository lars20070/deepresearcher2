...
The container build and startup process are tested in the CI pipeline defined in `.github/workflows/build.yaml`. The availability of major development tools and the successful execution of `uv run ruff check .` and `uv run pyright .` are verified. The container image is pushed to the GitHub Container Registry (GHCR) as `ghcr.io/lars20070/deepresearcher2-devcontainer`. Note that this image is for an `amd64` architecture only.
